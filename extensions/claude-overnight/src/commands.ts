...
export function createOvernightCommand(opts: {
  ...
  return {
    name: "overnight",
    description:
      "Spawn an unattended Claude Code session: `/overnight <prompt>`. Auto-resumes across 5h rate-limit windows.",
    acceptsArgs: true,
    requireAuth: true,
...
function resolveSupervisorScript(): string | undefined {
  ...
  const candidates = [
    ...
    path.join(here, "..", "claude-overnight", "scripts", "overnight-supervisor.cjs"),
    path.join(here, "..", "..", "claude-overnight", "scripts", "overnight-supervisor.cjs"),
    path.join(
      here,
      "..",
...
async function handleOvernight(
  ...
  if (!projectCwd) {
    return {
      text:
        "claude-overnight: projectCwd 未配置。设 env `OPENCLAW_CLAUDE_BRIDGE_CWD` " +
  ...
  if (!supervisor) {
    return { text: "claude-overnight: 找不到 supervisor 脚本（scripts/overnight-supervisor.cjs）" };
  }
  ...
  if (!botToken) {
    return {
      text:
        "claude-overnight: 环境里没有 TG_BOT_TOKEN —— 进度无法回报 Telegram。\n" +
  ...
  if (!chatId) {
    return { text: "claude-overnight: 无法解析 Telegram chat id" };
  }
  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        // Defensive: if supervisor ever shells out to `claude` and somehow
        // inherits a perm-hook from project settings, this short-circuits.
...
async function handleStatus(
  ...
  const projectCwd = resolveProjectCwd(config);
  if (!projectCwd) {
    return { text: "claude-overnight: projectCwd 未配置" };
  }
  const stateDir = resolveStateDir(projectCwd, config.stateDir);
