...
beforeEach(() => {
  ...
  delete process.env.AGENT_BRIDGE_TOPIC_MODE;
  delete process.env.AGENT_BRIDGE_FORUM_GROUP_ID;
});
...
describe("readTopicModeFromEnv", () => {
  ...
  it("accepts per-cwd and per-session", () => {
    expect(readTopicModeFromEnv({ AGENT_BRIDGE_TOPIC_MODE: "per-cwd" })).toBe("per-cwd");
    expect(readTopicModeFromEnv({ AGENT_BRIDGE_TOPIC_MODE: "per-session" })).toBe("per-session");
  });
  it("gives the canonical name precedence over the legacy fallback", () => {
    expect(
      readTopicModeFromEnv({
        AGENT_BRIDGE_TOPIC_MODE: "per-session",
        CLAUDE_BRIDGE_TOPIC_MODE: "per-cwd",
      }),
    ).toBe("per-session");
  });
...
describe("readForumGroupIdFromEnv", () => {
  it("returns undefined when empty / whitespace", () => {
    ...
    expect(readForumGroupIdFromEnv({ AGENT_BRIDGE_FORUM_GROUP_ID: "   " })).toBeUndefined();
  });
  it("trims and returns the value", () => {
    expect(readForumGroupIdFromEnv({ AGENT_BRIDGE_FORUM_GROUP_ID: "  -100123 " })).toBe("-100123");
  });
  it("gives the canonical name precedence over the legacy fallback", () => {
    expect(
      readForumGroupIdFromEnv({
        AGENT_BRIDGE_FORUM_GROUP_ID: "-100-new",
        CLAUDE_BRIDGE_FORUM_GROUP_ID: "-100-old",
      }),
    ).toBe("-100-new");
  });
});
