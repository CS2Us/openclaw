...
//   - "off" (default): same DM chat the /agent command came from
...
//   - "per-session": one forum topic per agent session id (max isolation;
...
// long-form provider tail streams route into the forum. This decouples
...
// daemon restart):
//   AGENT_BRIDGE_FORUM_GROUP_ID  — forum supergroup chat id (e.g. "-1001234")
//   AGENT_BRIDGE_TOPIC_MODE      — "off" | "per-cwd" | "per-session"
// Legacy CLAUDE_BRIDGE_* names are read-only fallbacks.
//
...
export type ResolveFollowTargetInput = {
  /** Source DM chat id (the chat that issued /agent). */
  dmChatId: string;
  cwd: string;
  sessionId: string;
  botToken: string;
...
const STORE_NAMESPACE = "agent-bridge.topic-routing";
...
function disable(error: unknown, op: string): void {
  ...
  try {
    runtimeRef?.logging
      .getChildLogger({ plugin: "agent-bridge", feature: "topic-routing" })
      .warn("agent-bridge topic routing store disabled", { op, error: String(error) });
  } catch {
...
export function readTopicModeFromEnv(env: NodeJS.ProcessEnv = process.env): TopicMode {
  const raw = (
    env.AGENT_BRIDGE_TOPIC_MODE?.trim() || env.CLAUDE_BRIDGE_TOPIC_MODE?.trim()
  )?.toLowerCase();
  if (raw === "per-cwd" || raw === "per-session") {
    return raw;
  }
  return "off";
}
...
export function readForumGroupIdFromEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const raw = env.AGENT_BRIDGE_FORUM_GROUP_ID?.trim() || env.CLAUDE_BRIDGE_FORUM_GROUP_ID?.trim();
...
export function buildTopicName(input: { mode: TopicMode; cwd: string; sessionId: string }): string {
  ...
  return path.basename(input.cwd) || "agent";
}
