// Discover agent session jsonl files for cross-process / cross-device session
// adoption (`/agent session` + `/agent continue` Telegram sub-commands).
//
...
// file's mtime advances. Same jsonl can be resumed by `agent --resume <uuid>`
 ...
 * blocks from a session jsonl. Used by the /agent switch panel to show
