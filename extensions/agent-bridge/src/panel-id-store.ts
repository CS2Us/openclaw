// Per-chat "current /agent panel" message id store.
//
// Goal: the /agent tab manager UI exists as exactly ONE message in the chat
// at any time. Every interaction (switch, newTab, follow, unfollow,
// enterAndFollow…) edits that one message in place. Re-issuing /agent
...
// path (command.ts) doesn't — when the user types /agent again there is
...
const STORE_NAMESPACE = "agent-bridge.panel-id";
...
function disable(error: unknown, op: string): void {
  ...
  try {
    runtimeRef?.logging
      .getChildLogger({ plugin: "agent-bridge", feature: "panel-id-store" })
      .warn("agent-bridge panel-id store disabled", { op, error: String(error) });
  } catch {
