...
describe("updateChatStateAfterTurn", () => {
  ...
  it("clears a provider-private session id when the tab changes provider", () => {
    createNewTab("telegram:1");
    updateChatStateAfterTurn("telegram:1", "claude-session", "claude");
    updateChatStateAfterTurn("telegram:1", null, "codex");
...
describe("hydrateChatStatesFromStore", () => {
  it("seeds in-memory state from persisted v2 records", async () => {
    ...
    // hydrate. User must explicitly re-enter a session via `/agent` after
    // daemon restart.
