...
describe("provider-aware configuration", () => {
  const canonical = process.env.OPENCLAW_AGENT_BRIDGE_CWD;
  const legacy = process.env.OPENCLAW_CLAUDE_BRIDGE_CWD;

  afterEach(() => {
    if (canonical === undefined) {
      delete process.env.OPENCLAW_AGENT_BRIDGE_CWD;
    } else {
      process.env.OPENCLAW_AGENT_BRIDGE_CWD = canonical;
    }
    if (legacy === undefined) {
      delete process.env.OPENCLAW_CLAUDE_BRIDGE_CWD;
    } else {
      process.env.OPENCLAW_CLAUDE_BRIDGE_CWD = legacy;
    }
  });

  it("gives the canonical cwd env precedence over the legacy reader and config", () => {
    process.env.OPENCLAW_AGENT_BRIDGE_CWD = "/canonical";
    process.env.OPENCLAW_CLAUDE_BRIDGE_CWD = "/legacy";
    expect(resolveProjectCwd({ projectCwd: "/config" })).toBe("/canonical");
  });

  it("keeps the legacy cwd as a read-only migration fallback", () => {
    delete process.env.OPENCLAW_AGENT_BRIDGE_CWD;
    process.env.OPENCLAW_CLAUDE_BRIDGE_CWD = "/legacy";
    expect(resolveProjectCwd({ projectCwd: "/config" })).toBe("/legacy");
  });
