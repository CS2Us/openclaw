...
export type PersistedTab = {
  ...
  /** Provider-private session ids may only be resumed by the same provider. */
...
const STORE_NAMESPACE = "agent-bridge.chat-state";
...
export function setAgentBridgeRuntime(runtime: PluginRuntime | undefined): void {
  runtimeRef = runtime;
...
function disable(error: unknown, op: string): void {
  ...
  try {
    runtimeRef?.logging
      .getChildLogger({ plugin: "agent-bridge", feature: "chat-state-store" })
      .warn("agent-bridge chat-state persistence disabled", {
        op,
        error: String(error),
      });
  } catch {
