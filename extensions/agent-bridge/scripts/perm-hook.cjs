#!/usr/bin/env node
// agent-bridge PreToolUse hook. Bridges every claude tool intent to the
...
// Env (set by agent-bridge before spawning claude):
...
async function bridge(input) {
  ...
  // own decision card directly so the UX lives 100% in agent-bridge (matches
    ...
    reply_markup: {
      inline_keyboard: [
        [
          ...
          // Handler lives in agent-bridge interactive.ts (action "ad").
...
// ---- follow-marker polling (matches openclaw/extensions/agent-bridge/src/follow-marker.ts) ----
        ...
        [
          {
            ...
            // lives in agent-bridge interactive.ts.
